...
MEASUREMENTS: Tuple[str, ...] = ("temperature", "humidity", "soil_humidity", "pH", "light", "light_natural")
...
class _StatsBase:
    ...
    def __init__(self, measurement_name: str):
        ...
        # getenv ONLY for these 2 (as requested)
        self.greenhouse_uuid = os.getenv('GREENBOX_UUID', cfg.get('greenhouse_uuid'))
        self.raspberry_uuid = os.getenv('RASPBERRY_UUID', cfg.get('raspberry_uuid'))

        # topics
        self.topic_publish = f"/{self.greenhouse_uuid}/{self.raspberry_uuid}/statistics/{self.measurement_name}"
        # sensors publish to: /{gh}/{rb}/sensors/<device_uuid>/<field>
        self.topic_subscribe = f"/{self.greenhouse_uuid}/{self.raspberry_uuid}/sensors/+/{self.measurement_name}"
...
class Statistics(_StatsBase):
    ...
    def __init__(self, measurement_name: str):
        ...
        from utils.tools import convert_list_to_df
...
class StatisticsHub:
    ...
    def __init__(self, sim: bool, measurements: Iterable[str] = MEASUREMENTS):
        self._workers = tuple(self._build_workers(sim, tuple(measurements)))

    @staticmethod
    def _build_workers(sim: bool, measurements: Tuple[str, ...]):
        if sim:
            # Sim version already accepts the measurement in __init__
            return tuple(StatisticsSim(m) for m in measurements)
        else:
            # DB-backed version expects MEASUREMENT as class attribute.
            # Define tiny on-the-fly classes and instantiate them.
            workers = []
            for m in measurements:
                cls = type(f"Stat_{m}", (Statistics,), {"MEASUREMENT": m})
                workers.append(cls())
